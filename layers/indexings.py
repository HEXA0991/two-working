...
def get_tag_indexing(config):
    ...
    if config.tag_form == 'iob2':
        # _Indexing = BIOIndexing(maxlen=config.maxlen, cased=True)
        _Indexing = BIOIndexing(cased=True)
    elif config.tag_form == 'iobes':
        _Indexing = BIOESIndexing(maxlen=config.maxlen, cased=True)
    else:
        _Indexing = Indexing(maxlen=config.maxlen, vocab_file=config.vocab_file, cased=True)
    indexing = _Indexing

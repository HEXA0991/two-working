...
class JointTrainer(Trainer):
    ...
    def _evaluate_during_train(self, model=None, trainer_target=None, args=None):
        ...
        with open(args.log_path, 'a') as f:
            ...
            e_f1 = f1
            f.write(f">> test entity prec:{precision:.4f}, rec:{recall:.4f}, f1:{f1:.4f}\n")
            ...
            r_f1 = f1
            f.write(f">> test relation prec:{precision:.4f}, rec:{recall:.4f}, f1:{f1:.4f}\n")
            ...
            r_f1_wNER = f1
            f.write(f">> test relation with NER prec:{precision:.4f}, rec:{recall:.4f}, f1:{f1:.4f}\n")
            if e_f1 > self.max_f1[0]:
                self.max_f1[0] = e_f1
                print('new max entity f1 on test!')
                
            if r_f1 > self.max_f1[1]:
                self.max_f1[1] = r_f1
                print('new max relation f1 on test!')
                
            if r_f1_wNER > self.max_f1[2]:
                self.max_f1[2] = r_f1_wNER
                print('new max relation f1 with NER on test!')
                
            if (e_f1 + r_f1) / 2 > self.max_f1[3]:
                self.max_f1[3] = (e_f1 + r_f1) / 2
                print('new max averaged entity f1 and relation f1 on test!')
                f.write('new max averaged entity f1 and relation f1 on test!\n')
